...
const PdfToJpg = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      // Convert PDF to JPG
      const jpgBlobs = await PDFService.pdfToJpg(file, setProgress);
      
      // Download the resulting JPG files
      PDFService.downloadBlobs(jpgBlobs, file.name.replace('.pdf', ''));
      
      toast({
        title: "Conversion complete",
        description: `Successfully converted ${file.name} to JPG images.`
      });
        ...
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center mb-6">
            <div className="bg-zenith-100 p-4 rounded-full">
              <ImagePlus size={32} className="text-zenith-600" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto">
            {!isConverting ? (
              <>
                ...
                <div className="mt-8">
                  <h2 className="text-lg font-semibold mb-4">How to convert PDF to JPG</h2>
