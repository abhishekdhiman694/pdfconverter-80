...
const PdfToPpt = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const pptBlob = await PDFService.pdfToPpt(file, setProgress);
      
      // Generate filename
      const fileName = file.name.replace('.pdf', '.pptx');
      
      // Download the PowerPoint file
      PDFService.downloadBlob(pptBlob, fileName);
      
      toast({
        title: "Conversion complete",
        description: `Successfully converted ${file.name} to PowerPoint format.`
      });
        ...
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center mb-6">
            <div className="bg-zenith-100 p-4 rounded-full">
              <Presentation size={32} className="text-zenith-600" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto">
            {!isConverting ? (
              <>
                ...
                <div className="mt-8">
                  <h2 className="text-lg font-semibold mb-4">How to convert PDF to PowerPoint</h2>
                  <ol className="list-decimal pl-5 space-y-2 text-zinc-700">
                    <li>Upload your PDF document.</li>
                    <li>Click the "Convert to PowerPoint" button.</li>
                    <li>Wait for the conversion to complete.</li>
                    <li>Download your PowerPoint presentation.</li>
                  </ol>
                </div>
              </>
            ) : (
