...
const PptToPdf = () => {
  ...
  const handleFileChange = (files: File[]) => {
    if (files.length > 0) {
      ...
      const isPpt = files[0].name.toLowerCase().endsWith('.ppt') || 
  ...
  const handleConvert = async () => {
    if (!file) {
      toast({
        ...
        description: "Please upload a PowerPoint file first."
      });
      return;
    }
    ...
    try {
      ...
      const pdfBlob = await PDFService.pptToPdf(file, setProgress);
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto">
            {!isConverting ? (
              <>
                ...
                <div className="mt-8">
                  <h2 className="text-lg font-semibold mb-4">How to convert PowerPoint to PDF</h2>
              ...
              <div className="py-12">
                <ConversionProgress 
                  progress={progress} 
                  fileName={file?.name || ''} 
                  conversionType="PowerPoint to PDF" 
                />
              </div>
            )}
          </div>
