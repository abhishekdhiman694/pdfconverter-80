...
const ExcelToPdf = () => {
  ...
  const handleFileChange = (files: File[]) => {
    if (files.length > 0) {
      ...
      const isExcel = files[0].name.toLowerCase().endsWith('.xls') || 
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const pdfBlob = await PDFService.excelToPdf(file, setProgress);
      ...
      toast({
        ...
        description: `Successfully converted ${file.name} to PDF format.`
      });
        ...
        <div className="container mx-auto px-4 text-center">
          <div className="flex justify-center mb-6">
            <div className="bg-zenith-100 p-4 rounded-full">
              <FileText size={32} className="text-zenith-600" />
            </div>
          </div>
          <h1 className="text-4xl font-bold mb-4">Excel to PDF Converter</h1>
          <p className="text-zinc-600 max-w-2xl mx-auto">
            Convert your Excel spreadsheets to PDF documents.
          </p>
        </div>
      </section>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto">
            {!isConverting ? (
              <>
                ...
                <div className="mt-8">
                  <h2 className="text-lg font-semibold mb-4">How to convert Excel to PDF</h2>
                  <ol className="list-decimal pl-5 space-y-2 text-zinc-700">
                    <li>Upload your Excel spreadsheet (.xls or .xlsx).</li>
